...
// Mobile bottom-nav shows every destination (horizontally scrollable).
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="hidden w-60 shrink-0 flex-col border-r border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 md:flex">
        <div className="mb-6 px-2 text-lg font-bold">Sameer's Job Agent</div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900 md:hidden">
          <span className="text-base font-bold">Sameer's Job Agent</span>
        ...
        {/* Mobile bottom nav */}
        <nav className="fixed bottom-0 left-0 right-0 z-40 flex items-stretch overflow-x-auto border-t border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 md:hidden">
          {MOBILE_NAV.map((n) => (
            <NavLink
              key={n.to}
              to={n.to}
              ...
              className={({ isActive }) =>
                cn(
                  "flex min-w-[64px] flex-1 shrink-0 flex-col items-center gap-0.5 py-2 text-[11px]",
